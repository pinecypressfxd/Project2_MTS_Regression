...
if __name__ == "__main__":
    ...
    # 'T_N_ratio']
    variables_Mom = ['Pp'#'Ni', 'Ne', 'plasma_beta',
    ...
    Pp_background = predicted_result_with_TSmodel['Pp_background'].values
    mask = ~np.isnan(Pp_background)
    Pp_background[np.isnan(Pp_background)] = 0# np.nanmedian(Pp_background)
    Pp_background_expend = np.expand_dims(Pp_background, axis=1)
    if method=='TS_model':
        X = np.concatenate((X,Pp_background_expend), axis=1)

    background_predicted = Pp_background[mask]
    background_observed = df.loc['Pp_Background_mean'].values[mask]

    fig = plt.figure(figsize=(6, 6))
    plt.scatter(background_observed, background_predicted, color='darkblue', alpha=0.3)
    plt.plot([df.loc['Pp_Background_mean'].min(), df.loc['Pp_Background_mean'].max()], [df.loc['Pp_Background_mean'].min(), df.loc['Pp_Background_mean'].max()], 'k--', lw=2)
    plt.xlabel(f'Observed Pp {unit}')
    plt.ylabel(f'Predicted Pp using TS model {unit}')
    plt.xlim([df.loc['Pp_Background_mean'].min(),df.loc['Pp_Background_mean'].max()])
    plt.ylim([df.loc['Pp_Background_mean'].min(),df.loc['Pp_Background_mean'].max()])
    ...
    plt.tight_layout()
    plt.show()
    fig.savefig(outputfilepath+'/predicted_result/Group2_'+y_variables[0]+'_features_with_background_TiTe_Flux_Entropy_dict.pdf',dpi=600,format='pdf')

    X[np.isnan(X)]=np.median(X)
    X[np.isinf(X)]=np.median(X)
    # 对特征值进行归一化
    ...
    plt.tight_layout()
    plt.show()
    fig.savefig(outputfilepath+'/predicted_result/Group2_'+y_variables[0]+'_predicted_result_using_B_'+method+'.pdf',dpi=600,format='pdf')
    ...
    # 将结果保存到 CSV 文件中
    results.to_csv(outputfilepath+'/predicted_result/Group2_'+y_variables[0]+'_predicted_result_using_B_'+method+'.csv', index=False)
