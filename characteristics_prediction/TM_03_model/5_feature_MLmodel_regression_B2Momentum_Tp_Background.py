...
if __name__ == "__main__":
    ...
    # 'T_N_ratio']
    variables_Mom = ['Ti'#'Ni', 'Ne', 'plasma_beta',
    ...
    Tp_background = predicted_result_with_TSmodel['Tp_background'].values
    mask = ~np.isnan(Tp_background)
    Tp_background[np.isnan(Tp_background)] = 0# np.nanmedian(Pp_background)
    Tp_background_expend = np.expand_dims(Tp_background, axis=1)
    if method=='TS_model':
        X = np.concatenate((X,Tp_background_expend), axis=1)

    background_predicted = Tp_background[mask]
    background_observed = df.loc['Ti_Background_mean'].values[mask]
    fig = plt.figure(figsize=(6, 6))
    plt.scatter(background_observed, background_predicted, color='darkblue', alpha=0.3)
    plt.plot([df.loc['Ti_Background_mean'].min(), df.loc['Ti_Background_mean'].max()], [df.loc['Ti_Background_mean'].min(), df.loc['Ti_Background_mean'].max()], 'k--', lw=2)
    plt.xlabel(f'Observed Tp {unit}')
    plt.ylabel(f'Predicted Tp using TS model {unit}')
    plt.xlim([df.loc['Ti_Background_mean'].min(),df.loc['Ti_Background_mean'].max()])
    plt.ylim([df.loc['Ti_Background_mean'].min(),df.loc['Ti_Background_mean'].max()])
