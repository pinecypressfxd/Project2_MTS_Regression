...
if __name__ == "__main__":
    ...
    filename = outputfilepath + '/data/'+'all-bubble-'+str(year_start)+'-'+str(year_end-1)+'_features_with_background_TiTe_Flux_Entropy_dict_del_483.pkl'# _del_483
    # 从文件中加载字典
    ...
    # 将结果保存到 CSV 文件中
    results.to_csv(outputfilepath+'/predicted_result/Group2_'+y_variables[0]+'_evaluation_MAPE_RMSE_result_using_B.csv', index=False)
    ...
    plt.tight_layout()
    plt.show()
    fig.savefig(outputfilepath+'/predicted_result/Group2_'+y_variables[0]+'_predicted_result_using_B.pdf',dpi=600,format='pdf')
    ...
    # 将结果保存到 CSV 文件中
    results.to_csv(outputfilepath+'/predicted_result/Group2_'+y_variables[0]+'_predicted_result_using_B.csv', index=False)
