...
if __name__ == "__main__":
    ...
    variables_Mom = [
                'Vx', 'Vy', 'Vz', 'Vi_total', 
                'Ni', 'Ne', 'Ti', 'Te', 'Pp', 'T_N_ratio', 'Ti_Te_ratio', 'ion_Special_Entropy', 'ele_Special_Entropy','Pos_X', 'Pos_Y', 'Pos_Z'
                ]
    ...
    # B
    target_variables = ['B_total'
                        # 'Bx', 'By', 'Bz', 'B_total', 'B_inclination', 'Pm', 
                   ]
    unit = '[nT]'
    # unit = '[nPa]'
    ...
    features_new = ['Mean','Median','Standard Deviation','Min','Max','Range','1st Quartile','3rd Quartile','Background_mean']# ,'Background_mean'
    ...
    df = df[~df.index.str.contains('seconds')]
     ...
     # 提取用于预测的变量数据
    X_variables = variables_Mom
    X_features =  features_new#['Max','Range']
    y_variables = target_variables# variables_B = ['Bx', 'By', 'Bz','B_total', 'B_inclination']
    ...
    # 将结果保存到 CSV 文件中
    results.to_csv(outputfilepath+'/predicted_result/Group3_'+y_variables[0]+'_evaluation_MAPE_RMSE_result_using_Moment.csv', index=False)
    ...
    plt.tight_layout()
    plt.show()
    fig.savefig(outputfilepath+'/predicted_result/Group3_'+y_variables[0]+'_predicted_result_using_Moment.pdf',dpi=600,format='pdf')
    ...
    # 将结果保存到 CSV 文件中
    results.to_csv(outputfilepath+'/predicted_result/Group3_'+y_variables[0]+'_predicted_result_using_Moment.csv', index=False)
