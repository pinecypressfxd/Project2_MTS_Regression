    ...
    for i_var in range(1,len(variance_type)-1):
        for i_normalized in range(0,len(normalized_type)-2):
            ...
            for i in range(int(np.shape(all_variables)[1]/240)):
                ...
                plt.plot(np.diff(y))
                plt.plot(np.diff(moving_average(y,mv_windows)))
                # plt.plot(np.diff(y))
                # plt.plot(moving_average(np.diff(y),mv_windows))
            ...
            plt.figure()
            # plt.plot(x)
            # plt.plot(moving_average(x,mv_windows))
            plt.plot(np.diff(x))
            plt.plot(np.diff(moving_average(x,mv_windows)))
            # plt.plot(np.diff(x))
            # plt.plot(moving_average(np.diff(x),mv_windows))
            ...
            '''滑动平均
            r_moving_average_all_bubble = []
            mic_moving_average_all_bubble = []
            for i_bubble in range(0,np.shape(all_variables)[0]):
                if i_bubble%100 == 0:
                    print('i_bubble: ',i_bubble,'percent:',i_bubble/np.shape(all_variables)[0])
                r_per_bubble = []
                ...
                for i in range(int(np.shape(all_variables)[1]/240)):
                    ...
                    # x = np.diff(all_targets[i_bubble])
                    # y = np.diff(all_variables[i_bubble,0+(240*i):240+(240*i)])
                    x = moving_average(x,mv_windows)
                    y = moving_average(y,mv_windows)
                    ...
                    r_per_bubble.append(r)
                    mic_per_bubble.append(mic)
                r_moving_average_all_bubble.append(r_per_bubble)
                mic_moving_average_all_bubble.append(mic_per_bubble)
            

            print('123')
            r_moving_average_all_bubble_array = np.array(r_moving_average_all_bubble)
            mic_moving_average_all_bubble_array = np.array(mic_moving_average_all_bubble)
            # 写入文件
            np.savetxt(fname=output_data_path+"/r_moving_average_all_bubble_array.csv", X=r_moving_average_all_bubble_array, fmt="%.2f",delimiter=",")
            np.savetxt(fname=output_data_path+"/mic_moving_average_all_bubble_array.csv", X=mic_moving_average_all_bubble_array, fmt="%.2f",delimiter=",")
            '''
            #exit(0)
            
            # 读取文件
            r_all_bubble_array = np.loadtxt(fname=output_data_path+"/r_moving_average_all_bubble_array.csv", dtype=np.float64, delimiter=",")
            mic_all_bubble_array = np.loadtxt(fname=output_data_path+"/mic_moving_average_all_bubble_array.csv", dtype=np.float64, delimiter=",")
            ...
            plt.savefig(output_data_path+'./Pearson r for all variables all bubbles.png')

            print('ccc')  

            #%% 
            # 读取文件
            # r_all_bubble_array = np.loadtxt(fname=output_data_path+"/r_all_bubble_array.csv", dtype=np.float64, delimiter=",")
            # mic_all_bubble_array = np.loadtxt(fname=output_data_path+"/mic_all_bubble_array.csv", dtype=np.float64, delimiter=",")
            #colors = ['#c72e29','#098154','#fb832d']
            colors = sns.color_palette("Paired")
