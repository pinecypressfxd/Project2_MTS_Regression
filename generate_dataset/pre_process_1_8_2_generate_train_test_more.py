...
if __name__ =='__main__':
    ...
    # # shape
    # positive_shape = [[2146,4321],[2146,2401],[2146,2401]]
    # negative_shape = [[1185,4321],[1185,2401],[1185,2401]]
    # negative_supplement_shape = [[119703,4321],[119703,2401],[119703,2401]]
    
    # shape
    # positive_shape = [[2490,4321],[2490,2401],[2490,2401]]
    # negative_shape = [[1359,4321],[1359,2401],[1359,2401]]
    # negative_supplement_shape = [[119703,4321],[119703,2401],[119703,2401]]
    
    # shape
    positive_shape = [[2668,4321],[2668,2401],[2668,2401]]
    negative_shape = [[1181,4321],[1181,2401],[1181,2401]]
    negative_supplement_shape = [[119703,4321],[119703,2401],[119703,2401]]
    #%% 正负样本比例
    ...
    for i_NP_ratio in negative_positive_ratio:
        for i_var in range(0,1):#len(variance_type)):
            for i_normalized in range(0,len(normalized_type)-1):
                #%%  目标数据格式
                # 数据中的变量(20)
                positive_sample_name = positive_negative_path + 'positive_bubble_data-add-2020-reevaluate-'+variance_type[i_var]+'-'\
                   +normalized_type[i_normalized]+'-shape_'+str(positive_shape[i_var][0])+'_'+str(positive_shape[i_var][1])+'.h5'
                negative_sample_name =  positive_negative_path + 'negative_bubble_data-add-2020-reevaluate-'+variance_type[i_var]+'-'\
                   +normalized_type[i_normalized]+'-shape_'+str(negative_shape[i_var][0])+'_'+str(negative_shape[i_var][1])+'.h5'
                negative_supplement_name =  positive_negative_path + 'negative_supplement_bubble_data-'+variance_type[i_var]+'-'\
                ...
                train_hdf5_outputfilename = train_test_path+'train_data-add-2020-622-reevaluate-'+variance_type[i_var]+'-'+normalized_type[i_normalized]+'-shape_'+str(np.shape(train_data_with_labels)[0])+'_'+str(np.shape(train_data_with_labels)[1])+'-NP_ratio_'+str(i_NP_ratio)+'.h5'
                test_hdf5_outputfilename = train_test_path+'test_data-add-2020-622-reevaluate-'+variance_type[i_var]+'-'+normalized_type[i_normalized]+'-shape_'+str(np.shape(test_data_with_labels)[0])+'_'+str(np.shape(test_data_with_labels)[1])+'-NP_ratio_'+str(i_NP_ratio)+'.h5'
                validation_hdf5_outputfilename = train_test_path+'validation_data-add-2020-622-reevaluate-'+variance_type[i_var]+'-'+normalized_type[i_normalized]+'-shape_'+str(np.shape(test_data_with_labels)[0])+'_'+str(np.shape(test_data_with_labels)[1])+'-NP_ratio_'+str(i_NP_ratio)+'.h5'
