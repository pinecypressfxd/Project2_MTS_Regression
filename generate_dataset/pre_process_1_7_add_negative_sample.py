...
file_path = 'D:/model_supplement_negative_data/'
data_file_path = file_path+'/data_samples/'
output_data_path = 'D:/preprocess_model_data/'

negative_sample_list_file = file_path + 'time_point_12min_interval/negative_list_non_plasma_sheet_after_sample_after_sorted_with_label.csv'
negative_sample_list = pd.read_csv(negative_sample_list_file)
negative_sample_list = negative_sample_list.drop(columns = 'Unnamed: 0')
...
for i_var in range(0,len(variance_type)):
    for i_normalized in range(0,len(normalized_type)):
        ...
        negative_hdf5_outputfilename = output_data_path+'negative_supplement_bubble_data-'+variance_type[i_var]+'-'+normalized_type[i_normalized]+'-shape_'+str(np.shape(data_negative)[0])+'_'+str(np.shape(data_negative)[1])+'.h5'
