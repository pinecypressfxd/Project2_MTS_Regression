...
for i_var in range(0,1):#len(variance_type)):
    for i_normalized in range(0,len(normalized_type)-1):
        ...
        negative_hdf5_outputfilename = output_data_path+'negative_supplement_bubble_data-add-2020-'+variance_type[i_var]+'-'+normalized_type[i_normalized]+'-shape_'+str(np.shape(data_negative)[0])+'_'+str(np.shape(data_negative)[1])+'.h5'
